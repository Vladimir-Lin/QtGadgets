...
N::HoloceneValidator:: HoloceneValidator ( QObject * parent )
                     : QValidator        (           parent )
                     , Validator         (           parent )
{
  Keywords [ 0 ] = tr ( "H.E."     ) ;
...
N::HoloceneValidator::~HoloceneValidator (void)
{
}

int N::HoloceneValidator::Type(void) const
{
  return 10000 + Calendars::Holocene ;
}

void N::HoloceneValidator::Fixup(QString & input) const
{
  fixup(input) ;
}

QLocale N::HoloceneValidator::Locale(void) const
{
  return QValidator::locale() ;
}

void N::HoloceneValidator::setLocale (const QLocale & locale)
{
  QValidator::setLocale(locale) ;
}

QValidator::State N::HoloceneValidator::isValid(QString & input,int & pos) const
{
...
QValidator::State N::HoloceneValidator::validate(QString & input,int & pos) const
{
  ...
  int     second                                    ;
  ///////////////////////////////////////////////////
  if (input.contains(Keywords[1]))                  { // check Before Holocene Era
    m     = input.left(Keywords[1].length())        ;
    okay  = ( m == Keywords[1] )                    ;
    index = Keywords[1].length()                    ;
  } else
  ///////////////////////////////////////////////////
  if (input.contains(Keywords[0]))                  { // check Holocene Era
