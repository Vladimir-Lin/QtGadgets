...
#ifdef WIN32
#pragma message("Buddhist calendar editor are required to do a check on leap years")
#else
#warning        "Buddhist calendar editor are required to do a check on leap years"
#endif

N::BuddhistEdit:: BuddhistEdit     ( QWidget * parent             )
                : CalendarTimeEdit (           parent             )
                , validator        ( new BuddhistValidator (this) )
{
...
N::BuddhistEdit::~BuddhistEdit(void)
{
}

QAbstractSpinBox::StepEnabled N::BuddhistEdit::stepEnabled(void) const
{
  return StepUpEnabled | StepDownEnabled ;
}

void N::BuddhistEdit::setRange(TimePeriod period)
{
  Range.L = period.L              ;
  Range.R = period.R              ;
  /////////////////////////////////
...
TUID N::BuddhistEdit::currentStardate(void)
{
  return Time . Stardate ;
}

bool N::BuddhistEdit::setStardate(TUID sd)
{
  Time . assign ( sd ) ;
  setTimeText   (    ) ;
  return true          ;
...
void N::BuddhistEdit::setTimeZone(int gmt)
{
  TimeZone = gmt ;
}

void N::BuddhistEdit::Adjustment(int year,int month,int mdays)
{
  BuddhistCalendar NX                    ;
  TUID             TT                    ;
  TUID             TS                    ;
...
bool N::BuddhistEdit::isLeap(int year)
{
  bool leap = false                               ;
  bool bc   = false                               ;
  int  ym                                         ;
...
void N::BuddhistEdit::AdjustDateTime(int year,int month,int mdays,int item)
{
  FixupDateTime ( year , month , mdays , item ) ;
}

void N::BuddhistEdit::stepBy(int steps)
{
  QLineEdit * le = lineEdit ( )             ;
  if (IsNull(le)) return                    ;
  QString s                                 ;
...
QValidator::State N::BuddhistEdit::validate (QString & input,int & pos) const
{
  return validator -> validate ( input , pos ) ;
}

void N::BuddhistEdit::setTimeText(void)
{
  QLineEdit * le = lineEdit ( )           ;
  if (IsNull(le)) return                  ;
  le -> blockSignals ( true             ) ;
  le -> setText      ( BuddhistString() ) ;
  le -> blockSignals ( false            ) ;
}

QString N::BuddhistEdit::BuddhistString(void)
{
  QString          t                                   ;
  BuddhistCalendar NX                                  ;
  int              Y                                   ;
...
bool N::BuddhistEdit::madeChange(const QString & input)
{
...
void N::BuddhistEdit::textChanged(const QString & text)
{
  QLineEdit * le = lineEdit ( )  ;
  if (IsNull(le)) return         ;
  int p = le->cursorPosition()   ;
